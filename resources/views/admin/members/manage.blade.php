            ...
            <div class="inputs">
                <div class="portlet-input input-inline">
                    <div class="input-icon right">
                        ...
                        <input name="search_query" id="search" type="text" class="form-control input-circle"
                               placeholder="{{ trans( 'members.fields.search.placeholder' ) }}">
                    </div>
                </div>
            </div>
        </div>
                ...
                <table class="table">
                    ...
                    @foreach( $users as $user )
                        <tr>
                            ...
                            <td> {{ $user->role }} </td>
                            <td>
                                <a class="btn red btn-outline" data-toggle="modal"
                                   href="#{{ $user->name }}_balance"> {{ trans( 'members.actions.give', ['currency' => settings( 'currency_name' )] ) }} </a>
                                <div class="modal fade" id="{{ $user->name }}_balance" tabindex="-1" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <form action="{{ url( 'admin/members/balance/' . $user->ID ) }}"
                                                  method="post">
                                                {!! csrf_field() !!}
                                                ...
                                                <div class="modal-footer">
                                                    <button class="btn dark btn-outline"
                                                            data-dismiss="modal">{{ trans( 'members.modal.close' ) }}</button>
                                                    <button type="submit"
                                                            class="btn green">{{ trans( 'members.modal.submit' ) }}</button>
                                                </div>
                                            </form>
                                        </div>
                                    </div>
