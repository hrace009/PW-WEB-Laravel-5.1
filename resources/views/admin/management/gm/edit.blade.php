...
@section( 'content' )
    <div class="portlet light bordered">
        <div class="portlet-body form">
            <form action="{{ url( 'admin/management/gm/edit/' . Request::segment( 5 ) ) }}" method="post"
                        ...
                        @foreach( $rights as $k => $v )
                            <div class="col-md-6">
                                <div class="md-checkbox">
                                    ...
                                    <input type="checkbox" id="right_{{ $k }}" class="md-check"
                                           name="gm_rights[{{ $k }}]"
                                           value="1" {{ isset( $user_rights[ $k ] ) ? 'checked' : NULL }}>
                                    <label for="right_{{ $k }}">
                                        <span class="inc"></span>
                                        <span class="check"></span>
                                        <span class="box"></span> {{ trans( 'management.gm_permissions.' . $k ) }}
                                    </label>
                                </div>
                            </div>
                        @endforeach
