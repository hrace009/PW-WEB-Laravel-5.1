        ...
        <div class="row">
            @if( $items->count() > 0 )
                @foreach( $items as $item )
                        ...
                        <form action="{{ url( 'shop/purchase/' . $item->id ) }}" method="post">
                            ...
                            <div class="grid-item box bg-white">
                                ...
                                <div class="grid-item-price font-blue">
                                    @if ( $item->discount)
                                        <s>{{ $item->price }}</s> <span
                                                class="font-red">{{ $item->price - ( ( $item->price / 100 ) * $item->discount ) }}</span> {{ settings( 'currency_name' ) }}
                                    @else
                                ...
                                <div class="caption mt-xs p-sm">
                                    <div class="scroller" style="height:100px" data-always-visible="1"
                                         data-rail-visible="1" data-rail-color="#111" data-handle-color="#000">
                                        {!! $item->description !!}
                                    </div>
                                    <div class="grid-item-buy text-center mt-md">
                                        @if ( $item->shareable )
                                            <button class="btn purple gift-button" data-toggle="modal"
                                                    data-target="#giftModal_{{ $item->id }}"><i
                                                        class="icon-paper-plane"></i> {{ trans( 'shop.gift' ) }}
                                            </button>
                                        @endif
                    ...
                    <!-- Modal -->
                    <div class="modal fade" id="giftModal_{{ $item->id }}" tabindex="-1" role="dialog"
                                ...
                                <div class="modal-content">
                                    <div class="modal-header">
                                        ...
                                        <h4 class="modal-title"
                                            id="myModalLabel">{{ trans( 'shop.gift_title', ['name' => $item->name]) }}</h4>
                                    </div>
                                    ...
                                    <div class="modal-footer">
                                        <button class="btn btn-primary " name="submit"
                                                type="submit">{{ trans( 'shop.send_gift' ) }}</button>
                                    </div>
                                </div>
                            </form>
                        </div>
