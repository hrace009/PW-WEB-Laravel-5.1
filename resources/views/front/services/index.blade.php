                                ...
                                @if ( is_array( trans( 'services.' . $service->key . '.input') ) )
                                    <div class="col-md-12">
                                        <div class="input-group">
                                            ...
                                            <input name="{{ trans( 'services.' . $service->key . '.input')['name'] }}"
                                                   type="{{ trans( 'services.' . $service->key . '.input')['type'] }}"
                                                   class="form-control"
                                                   placeholder="{{ trans( 'services.' . $placeholder ) }}">
                                            <span class="input-group-btn">
                                                <button class="btn btn-primary"
                                                        type="submit">{{ trans( 'main.buy' ) }}</button>
                                            </span>
                                        </div>
                                    </div>
                                @else
